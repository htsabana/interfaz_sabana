# -*- coding: utf-8 -*-
"""
Created on Thu May 12 15:16:02 2022
...
def parameter_interface():
    ...
    st.write('Interfaz destinada a designar parámetros a los pacientes, desarrollado por ***Healthtracker***')
...
async def health_team_interface():
    ...
    st.write('Interfaz destinada al equipo de salud designado, desarrollado por ***Healthtracker***')
    ...
    for i in range(len(df_general)):
        ...
        with mid:
            ...
            if st.checkbox('Mostrar reporte de paciente: ' + str(df_general['id_pct'][i]),key=i):
...
def download_csv_file_interface():
    ...
    st.write('Interfaz destinada a la descarga del archivo tipo .csv con información de sensores, desarrollado por ***Healthtracker***')
...
##################################################################################################
    ...
    if assigns == 'Asignar sábana a paciente':
        ...
        form_3 = st.empty()
        form3 = form_3.form(key='my_form_4')
...
async def main():
    ...
    if st.session_state.state1 == 0:
        ...
        with col1:
            st.image('logo.png', width=90)
        with mid:
            st.markdown("<h1 style='text-align: center; color: black;'>Interfaz general</h1>", unsafe_allow_html=True)

        st.header("""
            **Seleccionar interfaces que se deseen visualizar**:
        """)
        int_checkbox5 = st.checkbox('Control de pacientes',key=1)
        int_checkbox6 = st.checkbox('Control de asignaciones',key=1)
        int_checkbox = st.checkbox('Parámetros pacientes (en desarrollo)',key=1)
        int_checkbox2 = st.checkbox('Visualizar pacientes',key=1)
        int_checkbox3 = st.checkbox('Descargar archivo con información de sensores',key=1)
        int_checkbox4 = st.checkbox('Sábanas activas',key=1)
        ...
        if int_checkbox:# or st.session_state.load_state:
            parameter_interface()
            
        if int_checkbox2: #or st.session_state.load_state:
